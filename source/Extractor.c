...
static size_t ExtractExecutableFilesFromDumpFile(const char* pNewExecutableBaseFilename, void* pDumpFileContents, long lDumpFileSize);
...
main(int argc, char* argv[])
{
	...
	printf("[DIR] Searching for valid '" CUCKOO_DUMP_FILE_EXTENSION "' files in the current directory... ");
	...
	for (int i = 0; i < iDirEntries; i++)
	{
		...
		{
			printf("failed to allocate a buffer of size 0x%08lX bytes to hold the contents of the file !\n", lFileSize);
			goto label_close_file;
		}
		...
		size_t nExtractedExecutableFiles = ExtractExecutableFilesFromDumpFile
		(
			ppDirEnt[i]->d_name,
			pFileContentsBuffer,
			lFileSize
		...
		printf("[FIL] Extracted %lu executable file%s from the file '%s' !\n",
			nExtractedExecutableFiles, (nExtractedExecutableFiles == 1) ? "" : "s", ppDirEnt[i]->d_name);
...
static void
PrintUsageAndExit(int iExitCode)
{
	printf("[SYS] Usage: ./CuckooXtractor [-h]\n");
	printf("\n");
	printf("[SYS] Extracts executable files from within Cuckoo generated '" CUCKOO_DUMP_FILE_EXTENSION "' files\n");
	printf("[SYS] that are in the same directory as the program.\n");
	printf("\n");
	printf("[SYS] -h\tPrints this help message to the console then exits the program\n");
...
static int
MkstempsDumpFileFilter(const struct dirent* pDirEnt)
{
...
static size_t
ExtractExecutableFilesFromDumpFile(const char* pNewExecutableBaseFilename, void* pDumpFileContents, long lDumpFileSize)
{
	size_t nExtractedExecutableFiles = 0;

	for (long l = 0; l <= lDumpFileSize - Image_GetDOSHeaderSize(); l++)
	{
		PIMAGE_DOS_HEADER pImageDOSHeader = (PIMAGE_DOS_HEADER)((unsigned char*)pDumpFileContents + l);
		if (!Image_IsDOSHeaderMagicValid(pImageDOSHeader))
			continue;

		long lCurrentOffset = l + pImageDOSHeader->e_lfanew;
		if (lCurrentOffset + Image_GetNTHeaderSignatureSize() + Image_GetFileHeaderSize() + Image_GetOptionalHeaderMagicSize() > lDumpFileSize)
			goto label_next_but_one_address;

		PIMAGE_NT_HEADERS32 pImageNTHeader = (PIMAGE_NT_HEADERS32)((unsigned char*)pImageDOSHeader + pImageDOSHeader->e_lfanew);
		...
		if (File_SaveExecutableFileToDisk(pNewExecutableBaseFilename, pImageDOSHeader, uiSizeOfImage, l))
